...
const navItems = [
  ...
  { key: 'owners', path: '/owners', icon: Users },
  ...
  { key: 'inbox', path: '/communications', icon: Mail },
  { key: 'reports', path: '/reports', icon: BarChart3 },
  { key: 'channels', path: '/channels', icon: Radio },
  { key: 'loyalty', path: '/loyalty', icon: Heart },
  { key: 'affiliates', path: '/affiliates', icon: UserPlus },
];
...
export default function DashboardLayout() {
  ...
  const { sidebarOpen, toggleSidebar, setSidebarOpen, locale, setLocale, theme, setTheme } = useUIStore();
    ...
    <div className="min-h-screen bg-surface flex">
      ...
      <aside
        className={`
          ...
          [dir='rtl'] &:lg:right-0 [dir='rtl'] &:lg:left-auto
        `}
      >
        {/* Logo */}
        <div className="flex items-center gap-3 px-5 py-6 border-b border-surface-container-high/50">
          ...
          {sidebarOpen && (
            <div className="overflow-hidden">
              ...
              <p className="text-[10px] text-on-surface-variant tracking-widest uppercase">Management</p>
            </div>
          )}
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          ...
          {/* New Report Button */}
          {sidebarOpen && (
            <button className="w-full mt-4 flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-medium text-on-secondary gradient-accent hover:opacity-90 transition-opacity">
              <Plus className="w-5 h-5 flex-shrink-0" />
              <span>New Report</span>
            </button>
          )}
          {!sidebarOpen && (
            <button className="w-full mt-4 flex items-center justify-center px-3 py-2.5 rounded-lg text-on-secondary gradient-accent hover:opacity-90 transition-opacity">
      ...
      <div className="flex-1 flex flex-col min-h-screen">
        ...
        <header className="sticky top-0 z-30 glass-card ambient-shadow px-4 lg:px-6 py-3 flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            {/* Add Property */}
            <button className="hidden md:flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium text-on-secondary gradient-accent hover:opacity-90 transition-opacity">
                ...
                <div className="absolute right-0 top-full mt-2 w-72 glass-card rounded-xl ambient-shadow p-4 z-50">
                  ...
                  <div className="space-y-3">
                    <div className="flex gap-3 p-2 rounded-lg hover:bg-surface-container-low transition-colors">
                      ...
                      <div>
                        <p className="text-xs text-on-surface">New booking received for Rothschild 45</p>
                        <p className="text-[10px] text-on-surface-variant mt-0.5">2 minutes ago</p>
                      </div>
                    </div>
                    <div className="flex gap-3 p-2 rounded-lg hover:bg-surface-container-low transition-colors">
                      <div className="w-2 h-2 rounded-full bg-warning mt-1.5 flex-shrink-0" />
                      <div>
                        <p className="text-xs text-on-surface">Maintenance request pending approval</p>
                        <p className="text-[10px] text-on-surface-variant mt-0.5">15 minutes ago</p>
                      </div>
                    </div>
                    <div className="flex gap-3 p-2 rounded-lg hover:bg-surface-container-low transition-colors">
                      <div className="w-2 h-2 rounded-full bg-success mt-1.5 flex-shrink-0" />
                      <div>
                        <p className="text-xs text-on-surface">Monthly report generated successfully</p>
